...
同じ`.docplaybook/`ディレクトリには、次のファイルも含まれます：
...
### 柔軟性：ローカルと`CI`で異なるモデル

ローカルの開発環境と`CI`で異なるプロバイダやモデルIDが必要な場合、`config.json`から`model`を外しておいてください。
...
これはサポートされていますが、通常はローカル実行と`CI`間で翻訳出力の一貫性が低下します。
