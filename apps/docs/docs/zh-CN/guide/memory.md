...
`DocPlaybook` 将翻译指南保存在 Markdown 中，以便智能体可以重复使用并且人类仍可编辑。
...
## `docplaybook bootstrap` 如何初始化记忆

对于已经包含已翻译页面的现有文档站点，`docplaybook bootstrap --langs ...` 会根据仓库中已有的源/目标对齐示例创建首批记忆文件。
