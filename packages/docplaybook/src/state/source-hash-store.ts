...
import { getSourceHashesPath, getStateDir } from '../config.js';
import { ensureDir, pathExists } from '../utils.js';

export class SourceHashStore {
  ...
  private async load(): Promise<Record<string, string>> {
    ...
    const statePath = getSourceHashesPath(this.workspaceRoot);
  ...
  private async save(state: Record<string, string>): Promise<void> {
    ...
    await fs.writeFile(getSourceHashesPath(this.workspaceRoot), `${JSON.stringify(state, null, 2)}\n`, 'utf8');
    this.state = state;
  }
}
